...
### All Shared (siRNA, EWSFLI1, EWS)

```{r venn partitions siRNA gene tabular view; siRNAxEWSFLI1xEWS GO Biological Process, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r setup enrichr including siRNA screen , include=FALSE}
...
if (websiteLive) {
    enrichedsiRNAxEWSFLI1 <- enrichr(listVennPartitionsiRNA[["(EWS::FLI1∩siRNA_genes)∖(EWS)"]], dbs)
}

if (websiteLive) {
    enrichedsiRNAxEWSFLI1xEWS <- enrichr(listVennPartitionsiRNA[["EWS∩EWS::FLI1∩siRNA_genes"]], dbs)
}

if (websiteLive) {
    enrichedEWSxsiRNA <- enrichr(listVennPartitionsiRNA[["(EWS∩siRNA_genes)∖(EWS::FLI1)"]], dbs)
}
...
```{r enrichr siRNAxEWSFLI1 Biological Process , include = FALSE}
...
if (websiteLive) {
    plotEnrich(enrichedsiRNAxEWSFLI1$GO_Biological_Process_2015, showTerms = 20, numChar = 40, y = "Count", orderBy = "Adjusted.P.value", title = "siRNA and EWSFLI1 shared Enrichr")
}


```
```{r enrichr siRNAxEWSxEWSFLI1 Biological Processes, echo=FALSE, message=FALSE, warning=FALSE}


#Biological Process
if (websiteLive) {
    plotEnrich(enrichedsiRNAxEWSFLI1xEWS$GO_Biological_Process_2015, showTerms = 20, numChar = 40, y = "Count", orderBy = "Adjusted.P.value", title = "EWS Enrichr: GO Molec Function")
}

```

```{r enrichr siRNAxEWS Biological Processes, include = FALSE}


#Biological Process

if (websiteLive) {
    plotEnrich(enrichedEWSxsiRNA$GO_Biological_Process_2015, showTerms = 20, numChar = 40, y = "Count", orderBy = "Adjusted.P.value", title = "EWS Enrichr: GO Molec Function")
}

```

```{r enrichr gene tabular view; siRNAxEWSFLI1 GO Biological Process, include = FALSE}


siRNAxEWSFLI1_GOBiologProcess_DF <- enrichedsiRNAxEWSFLI1[["GO_Biological_Process_2015"]] %>% 
  select(-c(Old.P.value, Old.Adjusted.P.value, Odds.Ratio, Combined.Score)) %>%
  arrange(Adjusted.P.value)

knitr::kable(siRNAxEWSFLI1_GOBiologProcess_DF , col.names = gsub("[.]", " ", names(siRNAxEWSFLI1_GOBiologProcess_DF)), caption = "siRNAxEWSFLI1  GO Biological Process") %>%
   kable_styling("striped", full_width = F) %>% 
 scroll_box(width = "700px", height = "300px")
